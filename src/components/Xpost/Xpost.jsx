import React from "react";
import "./Xpost.css";
export default function Xpost() {
  return (
    <div className="container mx-auto mt-20">
      <h1 className="tips-video-sec-head last-cont-sec-head ">
        You are just one{" "}
        <span className="text-secondary">Startupathon Challenge</span> away from
        an exciting career as the{" "}
        <span className="text-secondary">founder of a company</span>, with a{" "}
        <span className="text-secondary">full salary</span> and{" "}
        <span className="text-secondary">ownership</span> in what you build.
      </h1>
      <div className="last-sec-photo-wrap">
        <div className="code-embed-22 w-embed w-script">
          <div
            className="twitter-tweet twitter-tweet-rendered flex max-w-[550px] w-full mt-5 mb-5"
            // style="display: flex; max-width: 550px; width: 100%; margin-top: 10px; margin-bottom: 10px;"
          >
            <iframe
              id="twitter-widget-0"
              scrolling="no"
              frameborder="0"
              allowtransparency="true"
              allowfullscreen="true"
              className="static visible w-[550px] h-[552px] block flex-1"
              //   style="position: static; visibility: visible; width: 550px; height: 552px; display: block; flex-grow: 1;"
              title="X Post"
              src="https://platform.twitter.com/embed/Tweet.html?dnt=false&amp;embedId=twitter-widget-0&amp;features=%3D%3D&amp;frame=false&amp;hideCard=false&amp;hideThread=false&amp;id=1814625998136029202&amp;lang=en&amp;origin=https%3A%2F%2Fpersistventures.com%2Fstartupathon%23mentor-network&amp;sessionId=9ebd1d19c565b3de7d53799d2abba3303ee93d1e&amp;theme=light&amp;widgetsVersion=2615f7e52b7e0%3A1702314776716&amp;width=550px"
              data-tweet-id="1814625998136029202"
            ></iframe>
          </div>{" "}
          <script
            async=""
            src="https://platform.twitter.com/widgets.js"
            charset="utf-8"
          ></script>
        </div>
      </div>
    </div>
  );
}
